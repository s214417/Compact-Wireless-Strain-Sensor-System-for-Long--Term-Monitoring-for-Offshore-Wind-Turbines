...
The first optimization stage adjusted only the turn diameter and serves as the reference against which the later stages are compared. The second stage is the line depth, which specifies how far the nozzle is lowered relative to the nominal sensing-layer height of \SI{0.2}{\milli\meter}, where a positive value brings the nozzle closer to the substrate and presses the deposited line more firmly while a negative value raises it, so that it sets the effective height and spread of the conductive line. The line-depth was found to have a particularly strong effect on the sensitivity in the small-strain regime.

The relative resistance change increases through the five stages for both materials, as summarized in \cref{tab:opt_results}, and the line depth produces the largest single improvement in each case. For the conductive PLA the turn-diameter-only stage barely responds, with a relative resistance change below \num{0.1}, which the line depth raises to about \num{0.26}, the largest step of the sequence. The line depth therefore has a particularly strong effect on the sensitivity in the small-strain regime.
