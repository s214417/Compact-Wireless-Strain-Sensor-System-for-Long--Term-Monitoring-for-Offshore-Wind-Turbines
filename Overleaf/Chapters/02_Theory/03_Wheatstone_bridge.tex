\pagebreak
\section{Wheatstone bridge} \label{sec:Wheatstone_theory}

\begin{figure}[!b]
\centering
\includestandalone[mode=buildmissing]{Standalone/Drawings/Wheatstone}
...
Replacing the fixed bias resistors with matched sensing elements offers a further benefit, namely the suppression of environmental drift. As these elements respond to the environment identically to the active sensors, resistance changes induced by environmental effects appear equally in both bridge arms and cancel in the differential output. 
Such temperature compensation is obtained whether the opposing arms are active, as in the full bridge, or passive elements that experience the environment but not the applied strain. Let the active and passive sensor resistances be modelled as:
...
Drift of the excitation voltage is suppressed in the same manner. In the bridge of \Cref{eq:bridge}, the output is the product of $V_{exc}$ and the relative unbalance $\left(R_b-R_s\right)/\left(R_b+R_s\right)$, a term that approaches zero as the bridge nears balance.. A drift in $V_{exc}$ therefore only scales this small differential signal, which constitutes a minor gain error. In the single divider, by contrast, the output is a fraction of $V_{exc}$, so the same drift shifts the operating point and appears directly as an offset error.
This residual gain error can be removed as well by deriving the ADC reference voltage from the excitation voltage. The output and the reference then drift together, so their ratio, and hence the converted reading, no longer depends on $V_{exc}$. The measurement is then fully ratiometric.