...
Drift of the excitation voltage is suppressed in the same manner. In the bridge of \cref{eq:bridge}, the output is the product of $V_{exc}$ and the relative unbalance $\left(R_b-R_s\right)/\left(R_b+R_s\right)$, which is small near balance. A drift in $V_{exc}$ therefore only scales this small differential signal, which constitutes a minor gain error. In the single divider, by contrast, the output is a fraction of $V_{exc}$, so the same drift shifts the operating point and appears directly as an offset error.
