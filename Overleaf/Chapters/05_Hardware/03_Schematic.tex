...
Two external crystal oscillators X1 and X2 have been added to the circuit even though the MCU already has four integrated clocks. The integrated clocks are RC oscillators and both resistance and capacitance varies with temperature more than the properties of quartz. Therefore it is favorable to add two external crystal oscillators to optimize the precision of the clocks over a wider range of temperatures. X1 is a high frequency 38.4 MHz crystal resonator \cite{X1_crystal} and X2 is a low frequency 32.768 kHz crystal resonator \cite{X2_crystal}.

\subsubsection{PMOS}
To reduce current consumption, a P-channel MOSFET (PMOS)~\cite{PMOS} is used to power-gate the sensors, connecting them to the \SI{3}{\volt} rail, VMCU, supplied by the LDO only when required. The device was selected for its ultra-low shutdown leakage current of 0.20 nA and its low quiescent current of 0.01 nA, both of which minimise wasted power. The switch is enabled through GPIO pin \texttt{PC00}, which is asserted high during data collection. Resistor R2 acts as a pull-down on the enable network and holds the switch disabled while the GPIO is high-impedance, for instance during an MCU reset, thereby preventing spurious activation and the associated current spikes. Powering the sensors only during data collection minimises their leakage contribution and reduces the overall power consumption.
...
The connectors used in this design are JST-SH board headers, selected for their compact footprint. Three are present. J1 is a 2-pin connector that accepts the battery and provides the input supply voltage. J2 is a 4-pin connector that forms the sensor interface and carries the sensor measurement signals. J3 is a 6-pin debug connector that exposes the serial wire debug (SWD) signals SWCLK, SWDIO and SWO, together with VDCDC, nRESET and GND.
...
Measurements of the VDCDC rail showed no difference in noise with and without the ferrite beads, and they were therefore removed. The absence of a measurable effect is attributed to the inherently low noise of the lithium thionyl chloride battery and to the effective attenuation of switching noise already provided by the LC output filter of the DC-DC converter.
