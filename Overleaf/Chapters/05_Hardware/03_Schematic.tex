...
The complete initial schematic, designed in \textit{Altium Designer Professional}, is shown in \cref{fig:Full_Schematic}, where the design is partitioned into four functional blocks. The microcontroller forms the core of the circuit together with its associated power regulation, and around it are arranged the Wheatstone bridge sensing circuit with its PMOS power-gating, the 2.4\,GHz RF front end, and the battery input with the LDO voltage regulator.
...
A higher supply voltage for the radio is not necessary in this design, as the selected device is limited to an output power of \SI{6}{\dBm}. An elevated PAVDD becomes relevant only for variants rated at \SI{14}{\dBm} and above. The input and output stages can likewise operate at \SI{1.8}{\volt}, since no GPIO in the system is required to function at \SI{3}{\volt}. The output of the buck converter is conditioned by an LC low-pass filter formed by inductor L4 and the shunt capacitor C9. As the converter switches the node VREGSW between the input supply and ground, the filter averages the resulting waveform into a steady \SI{1.8}{\volt} rail.
...
Unlike a conventional successive-approximation register (SAR) ADC,
which converts a single sampled value in one step, an incremental ADC
resets its internal state between conversions and employs oversampling and
digital filtering across multiple cycles to suppress noise and achieve a
higher effective resolution, at the cost of conversion speed.
...
Matching Guide' \cite{Matching_guide} and 'UG582: EFR32xG22E Explorer Kit User's Guide' \cite{User_guide}. Silicon Labs recommends using a T-filter for layouts using the EFR32MG22E chip, since it is less sensitive to layout variations compared to a PI-filter and achieves the best radiated harmonic margin for boards with a distance between the top layer and the first GND layer smaller than 800 $\upmu$m. Due to bonding wires and parasitics, Silicon Labs recommends an optimum termination impedance of $37 + j5\, \Omega$ at the pin RF2G4\_IO. For ease of design, the impedance matching circuit chosen for this design is highly inspired by the design Silicon Labs uses for their explorer kit with the same 2.4 GHz 50 $\Omega$ antenna \cite{Antenna}. \crefrange{eq:ZL1}{eq:Ztotal} are used to calculate impedance matching at the pin for this design. 
