...
The MCU selected for this implementation is the EFR32MG22E by Silicon Labs, a wireless System on Chip (SoC) integrating the microcontroller core and RF transceiver on a single die. In comparison with alternative candidates, the EFR32MG22E offers the lowest sleep current and the lowest TX current at \SI{6}{\dBm}, alongside one of the lowest unit prices at 14.32 DKK. Although it carries a modest price premium over the standard EFR32MG22, the variant is preferred for its significantly faster wake-up time from the lowest sleep mode, requiring only \SI{1.83}{\milli\second} against \SI{8.81}{\milli\second} for the standard part, a reduction by a factor of approximately \num{4.8}. This faster transition translates directly into a \SI{79.2}{\percent} saving in the energy consumed each time the device wakes, which contributes meaningfully to the overall battery lifetime budget.
...
An LDO converts the \SI{3.6}{\volt} battery voltage to a regulated \SI{3}{\volt}~\cite{LDO}. A stable supply is required because the accuracy of the IADC depends on a constant reference voltage, which an unregulated battery cannot guarantee since its terminal voltage varies with state of charge and temperature. The regulator was selected for its low quiescent current of \SI{1}{\micro\ampere} and its low dropout voltage of \SI{235}{\milli\volt}.
...
A higher supply voltage for the radio is not necessary in this design, as the selected device is limited to an output power of \SI{6}{\dBm}. An elevated \texttt{PAVDD} becomes relevant only for variants rated at \SI{14}{\dBm} and above. The input and output stages can likewise operate at \SI{1.8}{\volt}, since no General-Purpose Input/Outpu (GPIO) in the system is required to function at \SI{3}{\volt}. The output of the buck converter is conditioned by an LC low-pass filter formed by inductor L4 and the shunt capacitor C9. As the converter switches the node \texttt{VREGSW} between the input supply and ground, the filter averages the resulting waveform into a steady \SI{1.8}{\volt} rail.
...
Unlike a conventional Successive-Approximation Register (SAR) ADC, which converts a single sampled value in one step, an IADC resets its internal state between conversions and employs oversampling and digital filtering across multiple cycles to suppress noise and achieve a higher effective resolution, at the cost of conversion speed.
...
The MCU is programmed and debugged through the SWD interface exposed on connector J3. SWD is a two-wire alternative to the Joint Test Action Group (JTAG) standard, comprising \texttt{SWCLK}, which carries the clock, and \texttt{SWDIO}, which carries bidirectional data between the debugger and the target. Connector J3 additionally provides \texttt{SWO}, an optional single-wire trace output used for real-time logging, which may be omitted where trace functionality is not required.
...
The low-frequency crystal X2 was also omitted, as it is not used in the final firmware. A low-frequency crystal is primarily relevant for the low-power modes EM1 to EM3, as discussed in \Cref{sec:Power_Management_&_EM4_Deep_Sleep}. Since the firmware operates exclusively in Energy Mode 0 (run mode) EM0 and EM4, the high-frequency crystal X1 provides the clock reference in EM0, while the integrated Ultra-Low-Frequency RC Oscillator (ULFRCO) at \SI{1}{\kilo\hertz} is used in EM4, making X2 redundant.
