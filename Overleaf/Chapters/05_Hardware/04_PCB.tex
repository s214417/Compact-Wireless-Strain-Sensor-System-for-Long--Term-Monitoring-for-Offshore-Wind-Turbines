\section{PCB Design}
The PCB design is based on Silicon Labs' layout design guide AN928.2 \cite{Layout_guide} and follow the manufacturer specifications of \textit{JLCPCB} \cite{jlcpcb_capabilities}. Some alterations were made relative to the guide in order to ease assembly and rework, such as increased component spacing and larger solder pads. The initial design presented in \Cref{sec:Initial_PCB} was ordered from \textit{JLCPCB} and assembled, after which board-level modifications were made to match the final schematic circuitry. These modifications were subsequently incorporated into the final PCB design presented in \Cref{sec:final_pcb}.
...
\subsubsection{RF layout} \label{sec:rf_layout}
...
The revised design is shown in \cref{fig:PCB_REV7_VIEWS}, incorporating the modifications described in \cref{sec:Final_Schem}. Relocating the debug connector to the western edge of the board allowed an overall dimensions reduction of \SI{20.83}{\milli\meter} by \SI{25.02}{\milli\meter} with a component height of \SI{4.53}{\milli\meter}.
