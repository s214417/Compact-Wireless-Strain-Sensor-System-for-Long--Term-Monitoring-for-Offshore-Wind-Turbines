Once a measurement has been taken, the result is delivered to Home Assistant using the Zigbee Cluster Library (ZCL).
Each measurement cycle reports two quantities, namely the ADC value measured from the Wheatstone bridge and the on-die temperature of the microcontroller, where the latter is read from the always-on EMU temperature sensor and is included as a diagnostic of the operating conditions rather than as an external ambient measurement.
...
The temperature is reported on the Temperature Measurement cluster (0x0402) as the signed MeasuredValue attribute, expressed in hundredths of a degree Celsius, while the ADC value is reported on the Electrical Measurement cluster (0x0B04) as the unsigned RMS Voltage attribute.
The Electrical Measurement cluster is used as a convenient transport for the ADC value because it provides a standard unsigned analogue channel that Home Assistant exposes as a numeric entity, rather than because the value is a true RMS voltage.
...
The temperature is read from the EMU sensor in degrees Celsius and scaled up by a factor of one hundred to the hundredths of a degree expected by the signed MeasuredValue attribute, after which Home Assistant scales it back down from hundredths of a degree to degrees Celsius for display.
...
The ADC report is instead tracked, where its transmission-complete callback latches the outcome that the application state machine uses to decide whether the cycle has succeeded and the device may enter EM4, as described in \cref{sec:Measure-Report-Sleep_Cycle}.
...
This division reflects the relative importance of the two quantities, where delivering the ADC value is the essential purpose of each cycle, while the die temperature is at present only supplementary data that may be of use later, so it does not warrant holding the device awake to confirm.
