...
This cycle is implemented as a flat state machine inside \code{app\_process\_action()}, which the main loop calls once per iteration after the Zigbee stack has been serviced.
...
After a reset, the application initializes the analog front end, the Zigbee helper and arms the no-progress watchdog.
Control then passes to the main loop, where \code{zigbee\_process\_action()} drives the network bring-up while \code{app\_process\_action()} evaluates the state machine on every iteration.
...
When the firmware decides to sleep it calls \code{em4\_request()}, which only latches the request, and the entry into EM4 occurs on a later iteration once \code{zigbee\_ok\_to\_sleep()} confirms that the Zigbee stack has no pending work.
