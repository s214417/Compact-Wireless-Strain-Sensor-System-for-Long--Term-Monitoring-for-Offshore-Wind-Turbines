...
Rather than holding state across cycles, each cycle begins from a full reset, since waking from EM4 Shutoff restarts the MCU from boot.
The complete cycle, including the error-recovery branches, is shown in \cref{fig:measure_report_sleep}.

\begin{figure}[H]
\centering
\includestandalone[mode=buildmissing, width=0.9\textwidth]{Standalone/Drawings/Meas_Reap_Sleep.tex}
...
After a reset, the application initializes the analog front, the Zigbee helper and arms the no-progress watchdog.
Control then passes to the main loop, where \texttt{zigbee\_process\_action()} drives the network bring-up while \texttt{app\_process\_action()} evaluates the state machine on every iteration.
...
The firmware then waits for the transmission callback of the ADC report, which is the confirmation that the data has been acknowledged at the application support sublayer, and it is the event that gates the transition into deep sleep.
