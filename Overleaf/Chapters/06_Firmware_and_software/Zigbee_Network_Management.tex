...
This staged fallback, where a quiet restore is preferred first, a secure rejoin is attempted second, and a full leave and re-steer is used only as a last resort, avoids the cost of a full rejoin in the common case where the stored network is still present while still recovering automatically if the original network has become permanently unreachable.

For a Sleepy End Device, joining the network is not by itself enough to make the device usable by Home Assistant.
...
The first, \texttt{zigbee\_is\_network\_up()}, reports whether the device is joined and the security state is established, where a valid security context rather than mere association is treated as the point at which application traffic may begin, since the network can be associated before the network key is installed.
