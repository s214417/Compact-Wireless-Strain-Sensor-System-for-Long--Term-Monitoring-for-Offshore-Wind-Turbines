...
The IADC is clocked from the 20\,MHz FSRCO, with the analog conversion clock prescaled to nominal 10\,MHz.
...
The resulting full-scale ranges for each reference and gain combination are listed in \cref{tab:iadc_gain}, together with the voltage represented by a single converter code and the fraction of each range occupied by the expected bridge swing of approximately $\pm200$\,mV, which is derived from the bridge dimensioning in \cref{eq:swing_max} and the known resistance change of the sensors from (Section???)
...
The 32$\times$ oversampling and 16$\times$ digital averaging do not alter this step directly but lower the noise on the result, so that the smallest reliably resolvable voltage change approaches the quantitation step rather than being masked by noise.
...
The internal 1.2\,V bandgap reference is more stable in absolute terms and lower in noise, and at a gain of 4 its $\pm300$\,mV range matches the expected swing most closely, with the signal occupying roughly two-thirds of the range.
The 1.8\,V IOVDD reference instead makes the measurement fully ratiometric, because the bridge is excited from the same IOVDD rail through the GPIO output, so that drift in the supply scales the bridge output and the reference together and cancels in the result, as discussed in \Cref{sec:Wheatstone_theory}.
...
The larger full-scale range would become an advantage if the bridge swing were appreciably greater than $\pm200$\,mV, since a swing approaching or exceeding $\pm300$\,mV would clip against the 1.2\,V reference while the wider $\pm450$\,mV range of the IOVDD reference would still accommodate it.
For the present design, where the swing stays within $\pm200$\,mV, the firmware uses the internal 1.2\,V reference, prioritizing low per-sample noise and absolute accuracy, while the ratiometric IOVDD reference remains a viable alternative where long-term drift over the deployment lifetime is the dominant concern.
