The analog front end is implemented in \texttt{wheatstone.c}, which owns the bridge excitation and the IADC sampling and hides the pin routing and converter bring-up behind three calls used by \texttt{app.c}.
The Wheatstone bridge is excited from a single push-pull GPIO output on PC0 and measured differentially by the IADC, with PC1 as the positive input and PC2 as the negative input.
...
The gain is applied to the differential bridge signal by an amplifier stage ahead of the converter, so it scales the analog voltage before it is quantised rather than scaling the resulting integer.
...
The default-initialised structures are first populated with the chosen reference, analog gain, oversampling ratio, and digital averaging, after which the IADC is initialised and the differential input pair is routed to PC1 and PC2.
...
A measurement is taken by a single call to \texttt{wheatstone\_read()}, which enables the IADC, issues one single-shot differential conversion across PC1 and PC2, retrieves the result, and then gates the IADC clocks and the bridge excitation back off.
