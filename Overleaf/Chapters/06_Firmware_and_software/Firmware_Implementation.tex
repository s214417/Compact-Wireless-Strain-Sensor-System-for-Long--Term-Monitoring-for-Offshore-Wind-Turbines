...
\code{em4.c} owns the EM4 deep-sleep entry sequence, including Backup Real-Time Counter (BURTC) wake-up configuration.
All four modules are built on top of the Simplicity SDK, which provides the Zigbee stack, peripheral driver library (EMLIB), the sleep timer, and the Energy Management Unit (EMU) interface.
...
The resulting firmware image, comprising the four application modules together with the Zigbee stack and the SDK components on which they are built, occupies an address span of \SI{181.7}{\kilo\byte}, equivalent to \SI{35.5}{\percent} of the \SI{512}{\kilo\byte} of flash available on the EFR32MG22E. This substantiates the assessment made during MCU selection in \cref{tab:mcu_comparison}, where the comparatively light Zigbee stack was expected to keep the firmware footprint modest, and confirms that the selected device retains substantial storage headroom.
