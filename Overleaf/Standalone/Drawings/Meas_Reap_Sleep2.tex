...
\node (txq)     [decision]                    at (0, -11.2){TX callback};
...
\node (retry)   [box]                         at (4.0, -11.2){Schedule \\publish retry};
% Watchdog (left side)
...
\draw [arrow] (txq)     -- node[right] {success} (em4req);
...
\draw [arrow] (txq.east)   -- node[above] {fail} (retry.west);
