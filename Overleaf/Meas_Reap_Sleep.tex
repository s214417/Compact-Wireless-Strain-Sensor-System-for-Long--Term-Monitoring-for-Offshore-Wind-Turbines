...
\node[block, right=2.7cm of mcu, text width=2.2cm] (rf)
  {\textbf{RF front-end}\\
   {\footnotesize\itshape\color{black!55}(Chapter 5)}};

\node[block, right=2.2cm of rf, text width=3.0cm] (net)
  {\textbf{Wireless network}\\
   {\footnotesize\itshape\color{black!55}(Section 5.1)}};

\node[block, right=1.7cm of net, text width=2.6cm] (ha)
...
\node[draw=black!70, line width=1.1pt, rounded corners=6pt, fill=IBM5!50,
      inner sep=12pt, fit=(bridge)(mcu)(bat)(rf)] (nodebox) {};
\end{scope}
...
\draw[sig] (mcu.east) -- node[above, font=\footnotesize]{Radio transceiver} (rf.west);
\draw[sig, dashed] (rf.east) -- (net.west);
\draw[sig, dashed] (net) -- (ha);

\begin{scope}[shift={($(net.east)!0.5!(ha.west)+(0,0.5)$)}]
