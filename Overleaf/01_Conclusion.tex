...
The temperature is reported on the Temperature Measurement cluster (0x0402) as the signed \texttt{MeasuredValue} attribute, expressed in hundredths of a degree Celsius, while the ADC value is reported on the Electrical Measurement cluster (0x0B04) as the unsigned Root Mean Square (RMS) Voltage attribute.
...
The temperature is read from the on-die sensor in degrees Celsius and multiplied by one hundred to match the hundredths-of-a-degree resolution required by the signed \texttt{MeasuredValue} attribute, after which Home Assistant divides it back by one hundred to recover degrees Celsius for display.
