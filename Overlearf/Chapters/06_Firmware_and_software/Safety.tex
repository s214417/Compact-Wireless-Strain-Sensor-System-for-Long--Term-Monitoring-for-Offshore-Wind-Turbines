...
The first mechanism handles the common case in which a single report is not delivered, for instance because the network is not yet ready or because a transmission is interrupted or corrupted.
...
The second mechanism protects against the more serious case in which the active phase never completes at all, such as when a network join or the ZHA interview stalls and the device would otherwise stay awake waiting forever.
